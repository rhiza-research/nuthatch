...
class Cache():
    ...
    def __init__(self, config, cache_key, namespace, args, location, requested_backend, backend_kwargs):
        ...
        self.table_path = base_path.joinpath('nuthatch_metadata.delta')
    ...
    def _get_backend_from_metadata(self):
        backend_rows = QueryBuilder().register('metadata', self.dt).execute(f"""select backend from metadata where cache_key = {self.cache_key}
                                                                                AND namespace = {self.namespace}""").read_all()
