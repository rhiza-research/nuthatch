...
class DeltaBackend(FileBackend):
    ...
    def write(self, data):
        ...
        write_deltalake(self.path, write_data, mode='overwrite', schema_mode='overwrite', storage_options=self.config['filesystem_options'])
        return data
    ...
    def read(self, engine=None):
        if engine == 'pandas' or engine == pd.DataFrame or engine is None:
            return DeltaTable(self.path, storage_options=self.config['filesystem_options']).to_pandas()
        elif engine == 'dask' or engine == dd.DataFrame:
            return dd.from_pandas(DeltaTable(self.path, storage_options=self.config['filesystem_options']).to_pandas())
            #dd.from_polars(ps.read_delta(self.path, storage_options=self.config['filesystem_options'])
            #return ddt.read_deltalake(self.path, storage_options=self.config['filesystem_options'])
        else:
            raise RuntimeError("Delta backend only supports dask and pandas engines.")
