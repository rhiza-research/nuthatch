...
def pytest_generate_tests(metafunc):
    ...
    # @pytest.mark.cloud expands to all providers in CLOUD_PROVIDERS
    has_cloud = metafunc.definition.get_closest_marker("cloud") is not None
    all_markers = []
    filtered_providers = []
    providers = CLOUD_PROVIDERS if has_cloud else [p for p in CLOUD_PROVIDERS if metafunc.definition.get_closest_marker(p)]
